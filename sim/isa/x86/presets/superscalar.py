"""x86-32 N-wide superscalar pipeline preset.

x86 uses ByteInstructionMemory + X86Fetch with variable-length encoding.
For superscalar, we simplify: use word-aligned WideInstructionMemory with
X86Decoder per lane. This approximates superscalar x86 for educational purposes.

NOTE: In a real x86 superscalar, variable-length predecoding would determine
instruction boundaries before dispatching to lanes. Our simplified model
treats each 4-byte window as one instruction per lane, which works for
instructions that happen to be word-aligned (MOV r,imm32 is 5 bytes, etc.).
For the educational simulator, we use the standard pipeline instead for
fully accurate x86, and this superscalar preset provides the conceptual model.
"""
...
def build(program: list[int], num_lanes: int = 2, branch_predictor=None,
          dmem=None):
    """Build x86 superscalar preset.

    NOTE: For variable-length x86, this uses a simplified word-aligned model.
    The program should be provided as 32-bit word values for this preset.
    """
    ...
    # Use the same superscalar framework but with simplified decoders
    # For x86, we reuse RISCVDecoder as a placeholder since variable-length
    # decoding doesn't map cleanly to the N-wide fetch model.
    # A production design would use a predecoder stage.
    ...
    b.add("regfile",    WideRegisterFile(num_regs=8, zero_reg=False, num_lanes=num_lanes))
    b.add("hazard_det", CrossLaneHazardDetector(num_lanes=num_lanes, zero_reg_index=None))  # x86 EAX(0) is real
    b.add("id_ex",      WideID_EX(num_lanes=num_lanes))
    b.add("forwarding", WideForwardingUnit(num_lanes=num_lanes, zero_reg_index=None))  # x86 EAX(0) is real
    ...
    if branch_predictor.prediction_stage == "if":
        ...
        b.wire("bpred.prediction",                  "if_id.predicted_taken_in_0")
        b.wire("if_id.predicted_taken_out_0",       "id_ex.predicted_taken_in_0")
