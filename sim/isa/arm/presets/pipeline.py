"""ARM AArch64 5-stage pipeline preset.

Uses FlagsRegister for ARM condition-code branches (B.cond reads latched
flags from a prior SUBS/CMP, not the current ALU output).
"""
...
def build(program: list[int], branch_predictor=None, dmem=None, alu=None):
    ...
    b.add("if_id",       IF_ID(nop_encoding=0xD503201F))  # ARM NOP
    ...
    b.add("id_ex",       ID_EX())
    b.add("forwarding",  ForwardingUnit(zero_reg_index=31))  # ARM zero reg is XZR (31), not 0
    ...
    # Wiring is identical to RISC-V pipeline — only the decoder differs
    ...
    b.wire("decode.sets_flags",     "id_ex.write_flags_in")
    ...
    # Flags register (ARM condition codes for B.cond)
    ...
    # Pipeline the flags WITH the branch (see x86 pipeline preset): the
    # condition captured at the branch's own EX cycle travels through EX/MEM,
    # immune to younger flag-writers (ADDS/SUBS) reaching EX before resolve.
