"""
ARM AArch64 single-cycle preset.

Same shared components as RISC-V, swapping in the ARM decoder
and configuring the register file for 32 regs with zero register at index 31.

ARM uses a FlagsRegister (like x86) so that B.cond can read flags
set by a prior SUBS/CMP instruction rather than the current ALU output.
"""
...
def build(
    ...
    b.add("regfile",   StandardRegisterFile(num_regs=32, zero_reg=True, zero_reg_index=31))
    ...
    # ── Flags Register (ARM condition codes) ──────────────────────
    # SUBS/CMP sets flags; B.cond reads latched flags
    b.wire("alu.zero",          "flags_reg.alu_zero_in")
    b.wire("alu.result",        "flags_reg.alu_result_in")
    b.wire("decode.sets_flags", "flags_reg.write_flags")
    ...
    # ── Branch Resolution (reads from flags register, not ALU) ────
    ...
    if not configurable:
        ...
        # final cycle. (The flags register writes the same stable value each
        # held cycle, so it needs no gating.)
