"""ARM AArch64 N-wide superscalar pipeline preset."""
...
from sim.components.superscalar.wide_flags_select import WideFlagsSelect
...
def build(program: list[int], num_lanes: int = 2, branch_predictor=None,
    ...
    b.add("regfile",    WideRegisterFile(num_regs=32, zero_reg=True, zero_reg_index=31, num_lanes=num_lanes))
    b.add("hazard_det", CrossLaneHazardDetector(num_lanes=num_lanes, zero_reg_index=31))  # ARM XZR
    b.add("id_ex",      WideID_EX(num_lanes=num_lanes))
    b.add("forwarding", WideForwardingUnit(num_lanes=num_lanes, zero_reg_index=31))  # ARM XZR
    ...
    b.add("flags_select", WideFlagsSelect(num_lanes=num_lanes))
    b.add("flags_reg",  FlagsRegister())
    b.add("bpred",      branch_predictor)

    for i in range(num_lanes):
        b.add(f"decode_{i}",  ARMDecoder())
    ...
    eval_order.extend(["flags_select", "flags_reg", "branch", "dmem", "bpred"])
    b.set_eval_order(eval_order)

    # Wiring — identical structure to RISC-V superscalar
    ...
    for i in range(num_lanes):
        ...
        b.wire(f"decode_{i}.sets_flags", f"id_ex.write_flags_in_{i}")
    ...
    for i in range(num_lanes):
        ...
        b.wire(f"id_ex.write_flags_out_{i}",  f"ex_mem.write_flags_in_{i}")
    ...
    # Condition flags: select the youngest flag-setting lane (CMP/SUBS may land
    # in any lane), aligned at the EX/MEM stage — then drive the flags register.
    for i in range(num_lanes):
        b.wire(f"ex_mem.write_flags_out_{i}", f"flags_select.write_flags_{i}")
        b.wire(f"ex_mem.alu_zero_out_{i}",    f"flags_select.alu_zero_{i}")
        b.wire(f"ex_mem.alu_result_out_{i}",  f"flags_select.alu_result_{i}")
    b.wire("flags_select.alu_zero_out",   "flags_reg.alu_zero_in")
    b.wire("flags_select.alu_result_out", "flags_reg.alu_result_in")
    b.wire("flags_select.write_flags_out", "flags_reg.write_flags")
    ...
    # Data memory (multi-port — every lane has its own port)
    for i in range(num_lanes):
